...
public:
  Detector(image_transport::ImageTransport &it, ros::NodeHandle &nh) {
    ...
    this->signal_result_pub =
        nh.advertise<std_msgs::Int16>("/detector/signal/result", 1);

    this->cross_binary_pub = it.advertise("/detector/cross/binary", 1);
    this->cross_contour_pub = it.advertise("/detector/cross/contours", 1);
    this->cross_result_pub =
        nh.advertise<std_msgs::Int16>("/detector/cross/result", 1);

    this->red_light_binary_pub = it.advertise("/detector/light/red/binary", 1);
    this->red_light_contour_pub =
        it.advertise("/detector/light/red/contours", 1);

    this->yellow_light_binary_pub =
        it.advertise("/detector/light/yellow/binary", 1);
    this->yellow_light_contour_pub =
        it.advertise("/detector/light/yellow/contours", 1);
    this->light_result_pub =
        nh.advertise<std_msgs::Int16>("/detector/light/result", 1);
  }

  cv::Mat compressImg(cv::Mat &image, float ratio) {
    cv::Mat compressed;
    cv::resize(image, compressed,
               cv::Size(image.cols * ratio, image.rows * ratio));
    return compressed;
  }
  ...
  int detect_signal(cv::Mat &image) {
    ...
    this->signal_binary_pub.publish(
        cv_bridge::CvImage(std_msgs::Header(), "mono8", binary).toImageMsg());
    ...
    cv::drawContours(compressed, contours_result, -1, cv::Scalar(0, 255, 0),
                     10);
    this->signal_contour_pub.publish(
        cv_bridge::CvImage(std_msgs::Header(), "bgr8", compressed)
            .toImageMsg());
  ...
  void
  detect_traffic_binary(cv::Mat &binary, cv::Mat &gray, int min_contour_area,
                        float min_circle_ratio, int center_thresh,
                        std::vector<std::vector<cv::Point>> &contours_result) {
    std::vector<std::vector<cv::Point>> contours;
    cv::findContours(binary, contours, cv::RETR_EXTERNAL,
                     cv::CHAIN_APPROX_SIMPLE);
  ...
  int detect_traffic_lights(cv::Mat &image) {
    ...
    this->yellow_light_binary_pub.publish(
        cv_bridge::CvImage(std_msgs::Header(), "mono8", yellow_binary)
            .toImageMsg());

    auto yellow_contours = std::vector<std::vector<cv::Point>>();
    this->detect_traffic_binary(yellow_binary, gray, min_contour_area,
                                min_circle_ratio, center_thresh,
                                yellow_contours);
    cv::Mat yellow_contour_image = compressed.clone();
    cv::drawContours(yellow_contour_image, yellow_contours, -1,
                     cv::Scalar(255, 255, 0), 10);
    this->yellow_light_contour_pub.publish(
        cv_bridge::CvImage(std_msgs::Header(), "bgr8", yellow_contour_image)
            .toImageMsg());

    cv::Mat red_binary = this->red_binary(compressed, red_threshold);
    this->red_light_binary_pub.publish(
        cv_bridge::CvImage(std_msgs::Header(), "mono8", red_binary)
            .toImageMsg());

    auto red_contours = std::vector<std::vector<cv::Point>>();
    this->detect_traffic_binary(red_binary, gray, min_contour_area,
                                min_circle_ratio, center_thresh, red_contours);
    cv::Mat red_contour_image = compressed.clone();
    cv::drawContours(red_contour_image, red_contours, -1, cv::Scalar(0, 0, 255),
                     10);
    this->red_light_contour_pub.publish(
        cv_bridge::CvImage(std_msgs::Header(), "bgr8", red_contour_image)
            .toImageMsg());
  ...
  int detect_cross(cv::Mat &image) {
    ...
    cv::drawContours(compressed, contours_result, -1, cv::Scalar(0, 255, 0),
                     10);
    this->cross_contour_pub.publish(
        cv_bridge::CvImage(std_msgs::Header(), "bgr8", compressed)
            .toImageMsg());
...
int main(int argc, char **argv) {
  ...
  image_transport::Subscriber sub = it.subscribe(
      "/camera/rgb/image_raw", 1, boost::bind(imageCallback, _1, detector));
